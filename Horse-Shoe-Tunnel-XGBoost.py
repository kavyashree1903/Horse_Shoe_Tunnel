...
frprccolumns = pd.read_csv("horseshoetunnel.csv")
...
frprccolumns = frprccolumns[['x1','x2','x3','x4','y']]
...
def input_variable():
    x1 = st.sidebar.slider('H/D', float(frprccolumns.x1.min()), float(frprccolumns.x1.max()),
                               float(frprccolumns.x1.mean()))
    x2 = st.sidebar.slider('mi', float(frprccolumns.x2.min()), float(frprccolumns.x2.max()),
                           float(frprccolumns.x2.mean()))
    x3 = st.sidebar.slider('Sci/gD', float(frprccolumns.x3.min()), float(frprccolumns.x3.max()),
                            float(frprccolumns.x3.mean()))
    x4 = st.sidebar.slider('GSI', float(frprccolumns.x4.min()),
    ...
    res = {key : round(data[key], 2) for key in data}

    features = pd.DataFrame(res, index=[0])
    return features
...
st.table(df)
st.write('Here x1 = H/D, x2 = Mi, x3 = Sci/gD and x4 = GSI')
